...
import type { TokenSale, TokenSalesResponse } from '../types.ts';
...
export default {
  ...
  handler: async (
    ...
    try {
      ...
      const salesContext = composeContext({ state, template: getTokenSalesTemplate });
      ...
      const response = await axios.post<TokenSalesResponse>(config.STARGAZE_ENDPOINT, requestData, {
      ...
      const formatPrice = (price: number, symbol: string) =>
        `${Number(price).toLocaleString(undefined, { maximumFractionDigits: 2 })} ${symbol}`;
    ...
    Buyer: ${sale.buyer.address}`
        ...
        callback({
          ...
          content: { collection: content.collectionAddr, sales: sales }
        });
      }
